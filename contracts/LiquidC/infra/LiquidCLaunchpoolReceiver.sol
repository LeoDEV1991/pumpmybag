...
contract LiquidCLaunchpoolReceiver is Ownable {
    ...
    function _createLaunchPool(address _stakedToken, address _rewardToken, uint8 _durationDays) internal {
        ...
        // rewardToken or tokenXVault.want must be 'token'
        if (_rewardToken != token) {
            address vaultWant = ITokenXVault(_rewardToken).want();
